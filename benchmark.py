...
if __name__ == "__main__":
    ...
    class OTImputationIni(TransformerMixin):
        """imputation can be used to impute quantitative data and it relies on the idea that two batches extracted randomly from the same dataset should share the same distribution and consists in minimizing optimal transport distances between batches.
    ...
    result_saving_path = os.path.join(args.saving_path, f"psw_{args.dataset}")
    for n_round in range(args.n_rounds):
        ...
        logger.info(
            f"Round{n_round} - psw on {args.dataset}: MAE={mae:.4f}, MSE={mse:.4f}, MRE={mre:.4f}"
        )
